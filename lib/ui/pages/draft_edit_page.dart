...
class DraftEditPageState extends State<DraftEditPage> {
  ...
  final List<String> identityTypes = [
    'MyKad(NRIC)', 'Passport', 'Military / Police', 'Company / Registration Number'
  ];
  
  final List<String> titles = [
    'Mr.', 'Ms.', 'Mrs.', 'Dr.', 'JP', 'Pehin', 'Datin', 'Datuk', "Dato'",
    'Datuk Seri', 'Datin Seri', "Dato' Sri", 'Tan Sri', 'Puan Sri', 'Tun', 'Non applicable'
  ];
  
  final List<String> states = [
    'Johor', 'Perlis', 'Wilayah Persekutuan Putrajaya', 'Sabah',
  ...
  void _loadExistingData() {
    ...
    _emailController.text = widget.draftData['Email'] ?? '';
    ...
    _selectedFirstTimeBuyer = widget.draftData['FirstTime'] == 'Y' ? 'Yes' : 'No';
    ...
    if (widget.draftData['PaymentDate'] != null && widget.draftData['PaymentDate'].isNotEmpty) {
      _selectedPaymentDate = DateTime.tryParse(widget.draftData['PaymentDate']);
      if (_selectedPaymentDate != null) {
        _paymentDateController.text = DateFormat('dd/MM/yyyy').format(_selectedPaymentDate!);
      }
    }
  }
  ...
  void _updateDraft() async {
    ...
    final String draftGuid = widget.draftData['GUID']; // ✅ Required for update
    final String authorGuid = widget.draftData['AuthorGUID']; // ✅ Required for validation
    ...
    final Map<String, dynamic> updatedData = {
      ...
      'PaymentDate': _selectedPaymentDate != null
          ? DateFormat('yyyy-MM-dd HH:mm:ss').format(_selectedPaymentDate!)  // ✅ Convert to correct format
          : '',
    ...
    // // 🛠 Debug: Print all data before sending to API
    // print("\n🔍 DEBUG: Data Sent for Update:");
    ...
    try {
      ...
      if (response['status_code'] == 0) {
        // ✅ Success: Show confirmation message & return to previous page
          ...
          child: Column(
            children: [
              DropdownButtonFormField(
                ...
                items: identityTypes.map((type) => DropdownMenuItem(value: type, child: Text(type))).toList(),
                onChanged: (value) => setState(() => _selectedIdentityType = value),
              ),
              TextFormField(controller: _identityNumberController, decoration: const InputDecoration(labelText: "Identity Number")),
              DropdownButtonFormField(
              ...
              TextFormField(controller: _preferredNameController, decoration: const InputDecoration(labelText: "Preferred Name")),
              ...
              TextFormField(controller: _postCodeController, decoration: const InputDecoration(labelText: "Postcode")),
              ...
              DropdownButtonFormField(
                ...
                items: firstTimeBuyerOptions.map((option) {
                  return DropdownMenuItem(value: option['display'], child: Text(option['display']!));
                }).toList(),
                onChanged: (value) => setState(() => _selectedFirstTimeBuyer = value),
              ),
              TextFormField(
              ...
              TextFormField(controller: _agencyCompanyController, decoration: const InputDecoration(labelText: "Agency Company")),
              TextFormField(controller: _agentNameController, decoration: const InputDecoration(labelText: "Agent Name")),
              TextFormField(controller: _agentPhoneController, decoration: const InputDecoration(labelText: "Agent Telephone")),
              TextFormField(controller: _remarksController, decoration: const InputDecoration(labelText: "Remarks")),
              const SizedBox(height: 20),
              ElevatedButton(onPressed: _updateDraft, child: const Text("Update Draft")),
            ],
          ),
        ),
      ),
